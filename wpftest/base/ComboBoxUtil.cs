...
namespace WizMes_Alpha_JA
{
    class ComboBoxUtil
    {
        ...
        {
            ObservableCollection<CodeView> retunCollection = new ObservableCollection<CodeView>(); //리턴콜렉션 만들어놓고 
            
            DataTable dt = new DataTable();
            dt.Columns.Add("code_id");
            dt.Columns.Add("code_name");

            for (int i = 0; i < strValue.Count; i++) //strvalue는 N,Y 2개 
            {
                DataRow dr = dt.NewRow();
                dr["code_id"] = i; //i는 0
                dr["code_name"] = strValue[i]; //starvalue[0]는 N
        ...
        public ObservableCollection<CodeView> GetBankList()
        {
            ObservableCollection<CodeView> returnCollection = new ObservableCollection<CodeView>();

            string sql = " SELECT CB.BankID, CB.BankName  , CB.BankNameEng  , CB.Comments   , CB.Use_YN ,";
            sql += " Create_Date= CONVERT( VARCHAR(30), Create_Date, 120)   , Create_User_ID , ";
            sql += " Update_Date= CONVERT( VARCHAR(30), Update_Date, 120) , Update_User_ID  ";
            sql += " FROM [CM_BANK] CB  ";
            sql += "  WHERE 1 = 1   ";
            sql += " ORDER BY  CB.BankName ";
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        foreach (DataRow item in drc)
                        {
                            ...
                            returnCollection.Add(mCodeView);
                        }
                    }
                }
            }
